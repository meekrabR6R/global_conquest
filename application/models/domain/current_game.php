...
class CurrentGame{
	...
	*****************************************/
	public function getInitArmies($user_id){
        ...
        foreach($this->players as $player){
            ...
            if($user_id == $player['player']->getPlyrID())
                return $player['player']->getInitArmies();  
        }
    }
    ...
    *********************************************/
     public function armiesPlaced(){
        ...
        $set_flag = 0;
        foreach($this->players as $player){
            if($player['player']->getInitArmies() > 0)
                $set_flag = 1;
        }
	...
	**********************************/
	...
	public function getPlayerColors(){
		...
		$player_colors = array();
        foreach($this->players as $player)
            array_push($player_colors, array('plyr_id' => $player['player']->getPlyrID(), 'plyr_color' => $player['player']->getColor()));
